...
train<-read.csv("C:/Users/sandr/Documents/GitHub/BIG DATA/Taller3/scripts/train.csv")
test<-read.csv("C:/Users/sandr/Documents/GitHub/BIG DATA/Taller3/scripts/test.csv")

# Paquetes necesarios a cargar
...
color[train$apartamento == 1] <- "#a9d6e5"

color_test <- rep(NA, nrow(test))
color_test[test$casa == 1] <- "#012a4a"
color_test[test$apartamento == 1] <- "#a9d6e5"   

# Rescalar por año, los circulos mas grandes son más nuevos
...
grafica_train_completa <-  leaflet() %>% addTiles() %>%
  ...
  addCircles(lng = train$lon, 
             ...
             opacity = 1)%>%
  addPolygons(data=escuela_geometria , col="#f2542d")%>% #escuelas
  addPolygons(data=estacion_geometria , col="#f2542d")%>% #estaciones
  addPolygons(data=comercial_geometria , col="#0e9594")%>%#Centros comerciales
  addPolygons(data=banco_geometria , col="#f5dfbb")%>%#bancos
  addPolygons(data=parada_bus_geometria , col="#0d3b66")#parada de buses
  htmlwidgets::saveWidget(grafica_train_completa, "views//grafica_train_completa.html") 
  
# Gráfica completa base Test
